...
extern "C" void sendUARTCommand(int cmd)
{
    Serial.println(cmd);
}

extern volatile bool send_arm;
extern volatile bool send_band;

extern volatile int arm_value;
extern volatile int band_value;
...
void loop()
{
    ...
    if(Serial.available())
    {
        uint16_t valor = Serial.parseInt();

        if(valor == 100 || valor == 200 || valor == 300)
        {
            mostrar_figura_screen2(valor);
            mostrar_figura_screen6(valor);
        }
    }


    if(send_arm) /// it sends the command Arm: "value 0 up to 100"
    {
        Serial.printf("Arm:%d\n", arm_value);
        send_arm = false;
    }

    if(send_band) /// it sends the command Band: "value 0 up to 100"
    {
